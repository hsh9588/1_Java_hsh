...
//		int Num1; // Duplicate local variable Num1
		...
		int abcdefg;
		int showMeTheMoney;
		int ajfdklsajdflkwjermnenvzcxlkjasdklfjwqeiporuwit; // 길이 제한 X
...
//		int 1age; // Syntax error on token "1", delete this token
		...
		// 4. 특수문자는 '$', '_' 만을 허용한다.
		// -> '$' 는 내부 클래스 사용 시 자동으로 붙는 기호 ( 직접 사용 X )
		// -> '_' 는 상수 단어 구분 외에는 사용 X
...
//		long longNum!; // 에러
		...
		String str1;
		str1 = "안녕하세요?"; // str1 수의 초기화 ( 최초 값 대입 )
		...
		// 상수( final ) : 항[상] 같은 [수]
		// -> Java 에서는 한번 값을 기록하면 값을 바꿀 수 없는 변수
		
		// 일반 변수
		int num2 = 200;
		num2 = 300; // 새로 대입 가능
		
		// 상수 ( 상수는 대문자로 표기, 여러 단어면 _로 구분 )
