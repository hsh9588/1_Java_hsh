	...
	public void ex3() {
		...
		try {
			...
			// It is already handled by the catch block for Exception
											 ...
											 // 발생한 예외를 검사하여 알맞은 매개변수를 가진 catch에서 처리한다.
			
			// 스캐너를 이용한 입력 시 데이터 타입이 올바르지 않으면 발생하는 예외
			// ex) sc.nextInt(); 를 통한 입력시 정수가 아닌 값을 입력하면 발생
		...
		//	  -> 다형성( 업캐스팅 ) 이 적용 되어 해당 catch문에 처리된다.
	...
	public void ex4() {
		...
		try {
			ex5();  // throw new IOException();
			 ...
			 e.printStackTrace();
			// e : 예외 객체 참조 변수
			// printStackTrace : 예외가 발생한 메서드를 순서를 출력한다.
		}
	}
